...
namespace aslam
{
  namespace cameras
  {
          ...
          _tagCodes(AprilTags::tagCodes36h11)
    {
      if (tagSize <= 0.0)
      {
        throw std::runtime_error("tagSize has to be positive");
      }

      if (tagSpacing <= 0.0)
      {
        throw std::runtime_error("tagSpacing has to be positive");
      }
    ...
    bool GridCalibrationTargetAprilgrid::computeObservationInternal(
      ...
      if (detections.size() > 1)
      {
        for (unsigned i = 0; i < detections.size() - 1; i++)
          if (detections[i].id == detections[i + 1].id)
          {
            ...
            throw std::runtime_error("Found apriltag not belonging to calibration board");
          }
      }
